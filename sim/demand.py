...
def diurnal(
    base_lambda_vec: np.ndarray, hour: float, amp: float = 1.4, mu_am: float = 8, mu_pm: float = 18, sigma: float = 1.2
) -> np.ndarray:
...
def effective_lambda(
    base_lambda_vec: np.ndarray, hour: float, weather_fac: float = 1.0, event_fac_vec: np.ndarray = None
) -> np.ndarray:
