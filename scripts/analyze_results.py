...
def _to_csv(table: List[Dict[str, Any]], out_path: str) -> None:
    if not table:
        return
...
def _to_md(table: List[Dict[str, Any]], out_path: str) -> None:
    if not table:
        return
    cols = list(table[0].keys())
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
...
def main() -> None:
    ...
    files = sorted(list(dict.fromkeys(files)))  # dedupe, stable
    if not files:
    ...
    _to_csv(rows_out, str(args.out_csv))
    _to_md(rows_out, str(args.out_md))
    ...
    print("\n=== Top methods per scenario (ranked by mean of {}) ===".format(rank_key))
