...
\begin{abstract}
This report presents a realistic discrete-time simulation of an urban shared micro-mobility system for electric scooter fleet management. The simulator models stochastic demand with diurnal effects, weather variation, and localized events, as well as operational constraints such as station capacity, minimum state-of-charge (SoC) for rentals, charging delays, and relocation effort. We evaluate multiple controller families -- static planner baselines, a tick-level heuristic, episodic and contextual bandits, and a deep reinforcement learning agent (Soft Actor-Critic, SAC) -- across multiple network instances and scenario shifts.

Across networks, SAC is consistently among the top-performing methods and achieves the best or near-best objective in several configurations; however, tuned static baselines and the heuristic controller remain competitive in nominal conditions. Under scenario shift, long-horizon (168h) performance is primarily limited by queue accumulation under persistent spatial heterogeneity, while episodic bandits identify stable static operating modes but cannot adapt within an episode. Overall, the results emphasize that nominal performance alone is insufficient: robustness deltas and objective decomposition are necessary to reveal failure modes and trade-offs between service quality and operational cost.
\end{abstract}
...
Managing a shared e-scooter fleet is an operational control problem in which an operator must keep vehicles available across a city despite stochastic demand, spatial imbalance, battery depletion, charging delays, and finite relocation/charging resources. In practice, diurnal cycles, weather, and localized events can quickly drain stock at high-demand stations while other stations saturate, leading to unmet demand and persistent queues. Because these effects compound over time, nominal performance under ``typical'' conditions can mask fragile strategies that fail under scenario shift.

\paragraph{Modeling perspective}
The proposed simulator is primarily \emph{prescriptive}, supporting decisions on relocation and charging, it is also \emph{descriptive} through explicit state tracking (availability, queue backlog, SoC), \emph{predictive} in the sense that demand intensity evolves as a stochastic time-dependent process influenced by weather and events, and \emph{speculative} via ``what-if'' scenario stress tests. It is \emph{normative} only in the operational sense that policies are compared using an explicit objective that encodes a value judgement about service quality versus operational cost.

\paragraph{Problem statement}
Over a discrete horizon of $T$ time steps, the system state $\mathcal{S}_t$ summarizes fleet counts, battery SoC, and queue backlog across stations. At each step, a control policy selects an action $\mathbf{a}_t \in [0,1]^4$ that parameterizes relocation and charging planners, producing executable interventions that drive the transition $\mathcal{S}_t \rightarrow \mathcal{S}_{t+1}$ under stochastic demand and exogenous factors. The technical objective is to minimize the time-averaged normalized cost
\begin{equation}
J_{\mathrm{run}}=\frac{1}{T}\sum_{t=0}^{T-1}\left(
w_{U}\frac{U_t}{U_0}+w_{R}\frac{R_t}{R_0}+w_{C}\frac{C_t}{C_0}+w_{Q}\frac{Q_t}{Q_0}
\right),
\end{equation}
which trades off service quality (unavailability and queues) against operational effort (relocation distance and charging cost).

\paragraph{Aim, goals, research questions, and hypotheses}
The aim of this project is to provide a realistic and reproducible simulation-based testbed for scooter fleet management and to compare control strategies under uncertainty and scenario shift. Concretely, we (i) build a discrete-time simulator grounded in real city graphs and operational constraints, (ii) implement multiple controller families (static planners, an interpretable heuristic, bandits, and deep RL), and (iii) evaluate performance under both short (24h) and long (168h) horizons using robustness deltas and objective decomposition.
We address: (RQ1) whether tuned static planners remain competitive under nominal demand; (RQ2) which controllers are most robust under persistent heterogeneity and event shocks; (RQ3) how long-horizon evaluation changes ranking and failure modes; and (RQ4) the interpretability--robustness trade-off.
We hypothesize that (H1) SAC improves performance under sustained heterogeneity by adapting intervention intensity, (H2) queue accumulation dominates long-horizon failures, (H3) contextual bandits can adapt but may be high-variance on long horizons, and (H4) a well-designed heuristic can match top performance in benign regimes with greater transparency.

\paragraph{Paper structure}
Section \ref{sec:related_work} reviews related work; Section \ref{sec:materials_methods} describes the simulator, planners, and agents; Section \ref{sec:results} presents the experimental protocol and results; and Section \ref{sec:conclusion} concludes with limitations and future work.



\section{\label{sec:related_work}Related work}
Shared mobility fleet management is typically studied via optimization and simulation for rebalancing under stochastic demand, and via learning-based control for adaptive repositioning. Mijnster~\cite{mijnster2022sharedmobility_review} surveys simulation--optimization approaches for matching supply and demand, Qin et al.~\cite{qin2022rl_ridesharing_survey} review RL methods for mobility-on-demand control under non-stationarity and long-horizon feedback, and Villa-Zapata et al.~\cite{villazapata2024equity_rebalancing} highlight multi-objective formulations (e.g., efficiency and equity) that motivate explicit trade-off analysis.
...
\begin{equation}
  m_{i,t} \leftarrow m_{i,t} + k^{plug}_{i,t} \cdot \rho_i \cdot \frac{\Delta t}{60}.
\end{equation}
...
\begin{itemize}
  \item $U_t$: \textbf{Unavailability rate}. Defined as
  $U_t = 1 - \frac{\mathrm{served\_new}_t}{\max(1,\mathrm{demand}_t)}$,
  i.e., the fraction of new arrivals that cannot be served immediately at step $t$.
...
Each run produces a structured JSON file under \texttt{out/paper/<phase>/<network>/<method\_dir>/}, with one JSON file per scenario. Baseline planners are evaluated via \texttt{scripts.eval\_policy.py}; learning-based controllers and adaptive heuristics (Heuristic, UCB1, LinUCB, SAC) store both a method-level summary and, when applicable, traces (e.g., bandit arm selections). Aggregation is performed by \texttt{analyze\_results.py}, which scans \texttt{out/paper/**.json}, normalizes run schemas, and exports two primary artifacts: \texttt{master\_runs.csv} (objective and KPIs with mean$\pm$std over seeds) and \texttt{robustness.csv} (scenario deltas relative to Baseline for each \texttt{(phase, network, method)}).

Unless explicitly stated otherwise, the results in this section report only Phase A (24h) and Phase B (168h) evaluation outputs. Intermediate training/debug phases are excluded to avoid confounding evaluation statistics.

\subsection{\label{sec:results_metrics}Objective and reported metrics}

Methods are ranked by the time-averaged normalized objective over an episode:
\begin{equation}
J_{\mathrm{run}} = \frac{1}{T}\sum_{t=0}^{T-1}
\left(
w_{U} \frac{U_t}{U_0} + w_{R} \frac{R_t}{R_0} + w_{C} \frac{C_t}{C_0} + w_{Q} \frac{Q_t}{Q_0}
\right).
\end{equation}

where each component penalizes (i) unavailability, (ii) relocation effort, (iii) charging cost, and (iv) queue backlog, respectively. The normalization scales $(U_0,R_0,C_0,Q_0)$ are calibrated per network (Table~\ref{tab:env-configs}), and therefore absolute $J_{\mathrm{run}}$ values are most interpretable when comparing methods \emph{within the same network}, because normalization scales differ across networks (Table~\ref{tab:env-configs}).

In addition to $J_{\mathrm{run}}$, we report service and operational KPIs: availability $A=1-U$ (and the corresponding unavailability $U$), unmet-rate summaries, queue statistics, relocation distance, and charging cost. These KPIs are necessary to interpret cases where similar objective values arise via different trade-offs (e.g., lower cost but degraded service versus higher operational effort and stable service).
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/stacked_objective_phaseb_168h_porto10.png}
  \caption{Objective decomposition (mean over seeds) for Porto10, 168h. Stacked bars show mean contribution of unavailability, relocation, charging, and queue terms by method and scenario.}
  \label{fig:stacked_obj_porto10_168h}
\end{figure*}
...
\subsection{\label{sec:results_validation}Validation note: inactive sweep dimensions}

A counter-intuitive but important validation finding is that some sweep dimensions (notably charging-budget parameters in certain baseline families) produce negligible differences in aggregate outcomes. This suggests that the corresponding constraint is not binding under the current configuration (e.g., charging is limited by plug availability and time rather than budget), or that the planner pathway is effectively inactive in the tested regime. The correct scientific response is to report this invariance, avoid over-claiming sensitivity, and treat it as a concrete target for future validation and model refinement.
...
Future work should prioritize (i) operational realism and (ii) robustness under heterogeneity. On the modeling side, relocation should be constrained by explicit operational resources (truck capacity, routing, staffing, and time windows), and charging should be validated to ensure all control parameters are binding and influence behavior as intended (particularly where sweep invariances are observed).

On the control side, contextual bandits should incorporate stronger feature normalization and richer context (including graph-aware features) to reduce long-horizon variance and improve regime discrimination. For SAC, improvements include longer training budgets, curriculum training from easier to harder scenarios, and state representations that better capture network structure. Finally, the simulator can be extended with additional operational levers such as dynamic pricing, user incentives, and proactive repositioning, enabling new research directions in end-to-end micromobility optimization under uncertainty.
